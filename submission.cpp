...
public:
    ...
    void parse(FILE* inStream) {
        if (fscanf(inStream, "%d", &N) != 1) return;
        raw_rows.resize(N);
        for (int i = 0; i < N; ++i) {
    ...
    double interpolate(TaskType type, int batch_size) const {
        if (raw_rows.empty()) return 1.0;

        std::vector<std::pair<int, double>> points;
        for (const auto& r : raw_rows) {
            double val = -1.0;
            switch (type) {
                case TaskType::P_PRE: val = r.prefill_pre; break;
                case TaskType::P_PROC: val = r.prefill_proc; break;
                case TaskType::P_POST: val = r.prefill_post; break;
                case TaskType::D_PRE: val = r.decode_pre; break;
                case TaskType::D_PROC: val = r.decode_proc; break;
                case TaskType::D_POST: val = r.decode_post; break;
            }
            if (val >= 0.0) {
        ...
        if (batch_size <= points.front().first) return points.front().second;
        if (batch_size >= points.back().first) return points.back().second;
...
public:
    static std::vector<Task> resolveConflicts(const StateTracker& state, const std::vector<Task>& selectedTasks) {
        ...
        for (const auto& task : selectedTasks) {
            if (task.server == -1) {
                if (edgeUsed) continue;
                edgeUsed = true;
            } else if (task.server >= 0 && task.server < state.sysConfig.K) {
                if (cloudUsed[task.server]) continue;
...
public:
    static bool parseSystemConfig(SystemConfig& sys, FILE* inStream) {
        if (fscanf(inStream, "%d %lf %lf %lf %lld %d",
                   &sys.K, &sys.S, &sys.latency_in_ms,
                   &sys.bandwidth_gbps, &sys.bytes_per_token, &sys.num_layers) != 6) {
            return false;
        }
        return true;
    }
    ...
    static bool parseScoringConfig(ScoringConfig& sc, FILE* inStream) {
        if (fscanf(inStream, "%lf %lf %lf %lf %lf %lf %lf",
                   &sc.SLO1, &sc.SLO2, &sc.tp_UB,
                   &sc.tp_base, &sc.dist_base, &sc.w_tp, &sc.w_c) != 7) {
            return false;
        }
        return true;
    }
...
int main() {
    ...
    ScoringConfig sc;
    if (!ProtocolHandler::parseScoringConfig(sc, stdin)) return 0;
