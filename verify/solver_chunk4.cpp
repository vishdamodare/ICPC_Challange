...
public:
  ...
  std::vector<Task> selectTasks(const StateTracker &state,
    ...
    for (int k = 0; k < state.sysConfig.K; ++k) {
      ...
      if (!state.pProcReadyList[k].empty()) {
        int rid = state.pProcReadyList[k][0];
        int ls = state.requests[rid].nextLayerStart;
        int chunk_size = 4; // Chunk by 4 layers if decode is waiting
        int le = std::min(ls + chunk_size, state.sysConfig.num_layers);
        if (state.dProcReadyList[k].empty() || state.sysConfig.num_layers <= 4) {
          le = state.sysConfig.num_layers;
        }
        Task t;
        t.type = TaskType::P_PROC;
        t.server = k;
