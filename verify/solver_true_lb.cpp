...
public:
  ...
  std::vector<int> cloudAssignedCount;
  ...
  void init(const SystemConfig &sys) {
    ...
    cloudAssignedCount.assign(sys.K, 0);
  }
        ...
        if (ptr[0] == 'P' && ptr[1] == 'R' && ptr[2] == 'E') {
          ...
          if (rid >= 0 && rid < static_cast<int>(requests.size())) {
            if (le < sysConfig.num_layers) {
              ...
              if (cloudIdx >= 0 && cloudIdx < sysConfig.K) cloudAssignedCount[cloudIdx]--;
            }
          }
    ...
    for (int rid : task.requests) {
      if (rid >= 0 && rid < static_cast<int>(requests.size())) {
        ...
        case TaskType::P_PRE:
          ...
          if (task.remote >= 0 && task.remote < sysConfig.K) cloudAssignedCount[task.remote]++;
  ...
  std::vector<Task> selectTasks(const StateTracker &state,
    ...
    if (!state.edgeServer.busy) {
      if (!state.pPostReadyList.empty()) {
        ...
        int minCount = 1e9;
        for (int k = 0; k < state.sysConfig.K; ++k) {
          int count = state.cloudAssignedCount[k];
          if (count < minCount) {
            minCount = count;
            targetRemote = k;
          }
